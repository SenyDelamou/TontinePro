...
### 1.1 Contexte
TontinePro is a desktop tontine management application developed in Java Swing. It allows associations and savings groups to efficiently manage their members, contribution cycles, financial transactions and generate professional documents.

### 1.2 Objectifs
- Digitalize management of traditional tontines
- Automate balance and statistics calculations
- Generate professional member cards
- Ensure full audit trail of operations
...
### 2.5 Gestion des Prêts & Crédits
**Acteurs** : Administrateur, Gestionnaire

**Fonctionnalités** :
- **Nouveau Prêt** : Saisie du montant, taux d'intérêt et date d'échéance.
- **Suivi des Remboursements** : Calcul automatique des montants dus.
- **Statuts des Prêts** : En cours, Terminé, En retard.
- **Exportation** : Export des données de prêt en CSV.

**Règles métier** :
- Le membre doit être actif pour bénéficier d'un prêt.
- Le montant à rembourser inclut l'intérêt calculé.
...
- **Sidebar** : Bleu nuit, boutons orange avec bordure blanche, indicateur actif (flèche). Inclut désormais l'accès direct aux **Prêts**.
...
Voir fichier `tontinepro_database.sql` for schema detail.
